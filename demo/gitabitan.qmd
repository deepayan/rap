...
```{r}
power_trans <- function(x, p = 1)
{
    ## x should be between -1 and 1; not being checked
    sign(x) * abs(x)^p
}

notation2mp3 <- function(id, AFREQ = 440, L = 0.4, overtones = TRUE,
                         D = 0.25 * L, A = 0.1 * D, lowvol = 0.8)
{
    ...
    s <- cumsum1(w / R)
    W <- sin(2 * pi * s) |> power_trans(0.1)

    if (overtones) {
        ## add overtones at twice + thrice the frequncy 
        freqs2 <- within(freqs, {
            fstart <- 2 * fstart
            fend <- 2 * fend
        })
        ...
        s2 <- cumsum1(w2 / R)
        W2 <- sin(2 * pi * s) |> power_trans(0.2)
        w3 <- music2freq(freqs3, R)
        s3 <- cumsum1(w3 / R)
        W3 <- sin(2 * pi * s) |> power_trans(0.5)
        W[] <- W + 0.5 * W2 + 0.25 * W3
    }
    ...
    writeSignal(scaleSignal(W * V, qprob = 0.975), file = outfile, samp.rate = R)
}
```
