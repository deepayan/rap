...
x <- head(x, srate * 60) |> tail(srate * 20) # 20 seconds
...
```{r}
m <- splitSignal(x, patch = 1024, overlap = 512) # 1024 # ~ 23 milliseconds
system.time(acf.coefmat <- apply(m, 2, getACF, lag.max = 400, type = "correlation"))
system.time(pacf.coefmat <- apply(m, 2, getACF, lag.max = 400, type = "partial"))
```
...
```r
recons.ar <- generateAR(ar.coefmat, PATCHSIZE = 512,
                        innov.fun = rnorm, noise.sigma = 0.0005)
...
getARCoef <- function(x, ..., taper = FALSE)
{
    ...
    ar(x, aic = FALSE, ..., demean = FALSE, method = "yule")$ar
}

system.time(
    ar.burgmat <- apply(m, 2, getARCoef, order.max = 400)
)

recons.burg <- generateAR(ar.burgmat[,], PATCHSIZE = 512,
                          innov.fun = rnorm, noise.sigma = 0.005)
...
recons.levinson.ar <-
    generateAR(arcmat[,], PATCHSIZE = 512,
...
```{r}
recons.levinson.ma <-
    generateMA(macmat[,], PATCHSIZE = 512,
               innov.fun = rnorm, noise.sigma = 0.005)
...
```{r}
#| label: gen-ma
recons.ma <- generateMA(acf.coefmat, PATCHSIZE = 512,
                        innov.fun = rnorm, noise.sigma = 0.005)
```
...
$$
\rho_k = \theta_k + \theta_1 \theta_{1+k} + \dotsm + \theta_{q-k} \theta_q
$$

* Using this, recursively update (for $k = 1, 2, \dotsc, q$)

$$
\theta_k = \hat\rho_k - (\theta_1 \theta_{1+k} + \dotsm + \theta_{q-k} \theta_q)
$$
...
sillyMA <- function(rho, q = length(rho), niter = 10)
{
    ...
    updateTheta <- function() {
        theta[] <<- rho - sapply(1:q, function(k) sum(head(theta, -k) * tail(theta, -k)))
    }
...
#| label: silly-ma
system.time(
    silly.macmat <- apply(acf.coefmat, 2, sillyMA, q = 400, niter = 3)
)
str(silly.macmat)
```
...
```{r}
#| label: gen-ma-2
recons.sillyma <- generateMA(silly.macmat, PATCHSIZE = 512, 
                         innov.fun = rnorm, noise.sigma = 0.001)
```
