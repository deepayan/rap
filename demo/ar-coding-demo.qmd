...
system.time(acf.coefmat <- apply(m, 2, getACF, lag.max = 600, type = "correlation"))
system.time(pacf.coefmat <- apply(m, 2, getACF, lag.max = 600, type = "partial"))
```
...
system.time(
    ar.burgmat <- apply(m, 2, getARCoef, order.max = 600)
)
...
```{r}
library(gsignal)
arcmat <- t(levinson(acf.coefmat, p = 600)$a)[-1, ]
```
...
```{r}
macmat <- apply(m, 2, levinson.ma, p = 600)
```
...
```{r}
recons.levinson.ma <-
    generateMA(arcmat[,], PATCHSIZE = 512,
               innov.fun = rnorm, noise.sigma = 0.005)
...
x1 <- filterLP(recons.ma, lambda = 10)
```
